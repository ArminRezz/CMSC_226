...
void repListing() {
    ...
    for (int i = 0; i < SIZE; i++) {
        bookInfo(books[i].isbn, books[i].title, books[i].author, books[i].publisher, books[i].dateAdded, books[i].qtyOnHand, books[i].wholesale, books[i].retail, false);
    }
    string c = "";
...
void repWholesale() {
    ...
    for (int i = 0; i < SIZE; i ++) {
        ...
        cout << "Quantity On hand: " << books[i].qtyOnHand << endl;
        cout << "Wholesale Cost: " << books[i].wholesale << endl;
        total += books[i].wholesale * books[i].qtyOnHand;
        cout << "\n\n";
    }
...
void repRetail() {
    ...
    for (int i = 0; i < SIZE; i ++){
        if (books[i].title[0] != 0) {
            ...
            cout << "ISBN: " << books[i].isbn << endl;
            cout << "Quantity On hand: " << books[i].qtyOnHand << endl;
            cout << "Retail Price: " << books[i].retail << endl;
            total += books[i].retail * books[i].qtyOnHand;
            cout << "\n\n";
        }
    }
...
void repQty() {
    ...
    for (int i = 0; i < SIZE; i ++) {
        idPtr[i] = *qtyPtr[i];
        if (books[idPtr[i]].title[0] != 0) {
            cout << "Title: " << books[idPtr[i]].title << endl;
            cout << "ISBN: " << books[idPtr[i]].isbn << endl;
            cout << "Quantity On hand: " << books[idPtr[i]].qtyOnHand << endl;
            cout << "\n\n";
        }
    }
    string c = "";
...
void repCost() {
    ...
    //printing the sorted array
    for (int i = 0; i < SIZE; i ++) {
        idPtr[i] = costPtr[i]-&books[0].wholesale;
        if (books[idPtr[i]].title[0] != 0) {
            cout << "Title: " << books[idPtr[i]].title << endl;
            cout << "ISBN: " << books[idPtr[i]].isbn << endl;
            cout << "Wholesale Cost: " << books[idPtr[i]].wholesale << endl;
            cout << "\n\n";
        }
    }
    string c = "";
...
void repAge() {
    ...
    for (int i = 0; i < SIZE-1; i ++) {
        if (books[idPtr[i]].title[0] != 0) {
            cout << "Title: " << books[idPtr[i]].title << endl;
            cout << "ISBN: " << books[idPtr[i]].isbn << endl;
            cout << "Date Added: " << books[idPtr[i]].dateAdded << endl;
            cout << "\n\n";
        }
    }
    string c = "";
